...
class TaskPolicyLeanerAgent(ArbiAgent):
    def __init__(self):
        ...
        self.TA_name = "agent://www.arbi.com/TaskAllocator"
        self.index = 0
        self.processing = True
        self.robots = ["http://www.arbi.com/ontologies/arbi.owl#AMR_LIFT1", "http://www.arbi.com/ontologies/arbi.owl#AMR_LIFT2", "http://www.arbi.com/ontologies/arbi.owl#AMR_LIFT3", "http://www.arbi.com/ontologies/arbi.owl#AMR_LIFT4"]
       # self.robots = [
         #              "http://www.arbi.com/ontologies/arbi.owl#AMR_LIFT2"]
        self.contexts = ["robotAtVertex $Robot $Vertex", "batteryRemain $Robot $Battery", "distance $Robot $Cargo $Distance"]
        self.prefix= "http://www.arbi.com/ontologies/arbi.owl#station"
    ...
    def get_predicate(self, robot, goal):
        ...
        for context in self.contexts:
            ...
            for i in range(len(context)-1):
                if context[i+1] == "$Cargo":
                    msg= msg + " \"" +self.prefix+"{0}\"".format(goal)
        ...
        return pred
#??? who are you song ah ha wait witat
    def on_request(self, sender: str, request: str) -> str:
        ...
        split_req = request.split('(')[2]       
        msg_robot = split_req.split(')')[0]        
        robots = split_req.split(')')[0].split()[1:]       
        goal = split_req.split(')')[1][1:]
               
        #check robot
        alloc_robot = list()
        for r in robots:
            if r =="AMR_LIFT1":
                r = "http://www.arbi.com/ontologies/arbi.owl#AMR_LIFT01"
            elif r =="AMR_LIFT2":
                r = "http://www.arbi.com/ontologies/arbi.owl#AMR_LIFT02"                
            elif r =="AMR_LIFT3":
                r = "http://www.arbi.com/ontologies/arbi.owl#AMR_LIFT03"
            elif r =="AMR_LIFT4":
                r = "http://www.arbi.com/ontologies/arbi.owl#AMR_LIFT04"

            alloc_robot.append(r)
        ...
        if sender == self.TA_name:
            for r in alloc_robot:

                pred = self.get_predicate(r, goal)
                #pred = [] #for test 
        ...
        return self.resTA(msg_robot, safety_list, efficiency_list)
    
    
    def resTA(self, msg_robot, safety_list, efficiency_list):
        #print("response!!")
        #print("safety:", safety_list)
        #print("efficiency", efficiency_list)
        
        msg_safety = "(Safety"
        msg_efficiency = "(Efficency"
        
        for i in range(len(safety_list)):
            msg_safety = msg_safety +" " +str(safety_list[i])
            msg_efficiency = msg_efficiency + " " +str(efficiency_list[i])
            #print(msg_safety)
            #print(msg_efficiency)
                        
        msg = "(TaskPolicy ("+str(msg_robot)+") "+msg_safety+") "+msg_efficiency+ "))"
        print("responseTA =>", msg)
